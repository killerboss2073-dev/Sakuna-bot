...
# Bot configuration
BOT_TOKEN = "8225668512:AAHsHp8nsr1mvhCTWL3yb7SI5XDrz3hb638"
...
BET_SEQUENCE_777 = [10, 30, 70, 160, 320, 760, 1600, 3200, 7600, 16000, 32000, 76000]  # Steps for loss progression
...
def get_next_bet_amount_777(current_step):
    ...
    if current_step < len(BET_SEQUENCE_777):
        return BET_SEQUENCE_777[current_step] * 100  # Convert to actual amount
    else:
        return BET_SEQUENCE_777[-1] * 100  # Use last amount if sequence exceeded
    ...
    try:
        ...
        if result == 'WIN':
            ...
            if new_step >= len(BET_SEQUENCE_777):
                new_step = len(BET_SEQUENCE_777) - 1  # Stay at last step
            new_total_profit = total_profit + profit_loss
